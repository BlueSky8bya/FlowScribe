...
1. 절대금지 위반 (severity: critical)
   ...
   중요 — 판정 전 두 가지를 반드시 먼저 확인한다:

   [A] 항목이 실제 금지인지 확인
   - "~에서는 X가 있다/없다", "~는 Y이다" 형태의 세계 설명·사실 기술은 금지 항목이 아니다
   - 금지 항목은 행위·사건에 대한 명시적 제약("X를 하면 안 된다", "Y는 등장하지 않는다" 등)이다
   - 세계 설명문을 위반으로 판정하지 않는다

   [B] 조건부 규칙 해석
   - 항목에 조건 표현("~에서는", "~일 때는", "~상황에서는", "~배경에서는" 등)이 있으면
     현재 장면이 그 조건을 만족하는지 먼저 확인한다
   - 조건이 충족되지 않는 장면이면 해당 규칙을 적용하지 않는다
   - 현재 장면이 그 조건에 해당하는지 불분명하면 위반으로 판정하지 않는다
...
4. 시점 위반 (severity: major)
   시점별 판정 기준 — 반드시 입력된 시점 값을 먼저 확인한 뒤 해당 규칙만 적용한다:

   ■ 1인칭 주인공
     위반: 서술부에서 갑자기 "그가/그녀가" 등 3인칭으로 전환
     정상: 화자 자신의 감정·생각 서술, 대사 안의 모든 표현

   ■ 1인칭 관찰자
     위반: 서술부에서 타인의 감정·생각을 직접 단정 ("그는 두려웠다")
     정상: 화자 자신의 내면 서술, 대사 안의 표현

   ■ 3인칭 관찰자
     위반: 서술부에 "나는/나의/내가" 등장 / 인물 내면을 서술자가 직접 단정
           ("그녀는 X를 알고 있었다" "그는 Y를 결심했다" 등 지식·의도·생각 직접 확정)
     정상: 외부에서 관찰 가능한 행동·반응 — "망설였다" "굳었다" "눈살을 찌푸렸다" "발이 멈췄다"
           행동 묘사 후 독자가 감정을 유추할 수 있는 간접 서술
           대사 안의 1인칭 표현

   ■ 전지적 작가
     위반: 서술부에 "나는/나의/내가" 등장
     정상: 인물 내면(감정·생각·의도) 서술 — 전지적 시점이므로 내면 묘사는 위반 아님

   ■ 교차 시점
     위반: 동일 장면 내에서 시점 인물이 무작위로 뒤섞임 (장면 전환 없이)
     정상: 장면 전환마다 시점 인물이 바뀌는 것 — 전환 자체는 위반 아님
     대사 안 1인칭은 모든 시점에서 위반 아님
...
■ 하드 위반 (hard_violations) — 독자가 읽다가 명백히 이상한 오류만:
  ...
  • 설정된 시점 위반 — 시점 유형별로 다르게 판정:
    - 1인칭 주인공: 서술부에서 "그가/그녀가" 등 3인칭 전환
    - 1인칭 관찰자: 타인의 감정·생각 직접 단정 (화자 자신 내면은 정상)
    - 3인칭 관찰자: 서술부에 "나는/나의/내가" / 인물 내면을 직접 단정하는 서술
      ("망설였다" "굳었다" 등 외부 관찰 가능 반응은 정상)
    - 전지적 작가: "나는/나의/내가" 서술부 등장만 위반 — 내면 묘사는 정상
    - 교차 시점: 장면 전환 없이 동일 장면 내 시점 인물 혼재
    - 대사(따옴표 내부) 안의 1인칭은 모든 시점에서 위반 아님
  • 이전 상태와 명백한 모순: 죽은 인물 등장, 부상 부위(해당 팔/다리)를 직접 정상 사용,
    [인물 현재 상태]에 기록된 소지품에 없는 물건 사용
    (단, 부상/소지품 언급 안 함·부상과 무관한 동작·이번 화에서 획득한 물건 사용은 위반 아님)
  • 절대금지 항목 위반 — 판정 전 두 가지 확인:
    ① 세계 설명·사실 기술("~에서는 X가 있다" 등)은 금지 항목이 아님 → 위반 판정 불가
    ② 조건 표현("~에서는" "~일 때는" 등)이 있으면 현재 장면이 그 조건인지 먼저 확인,
       불충족·불분명하면 적용 안 함
  • ※ 시공간 연결성은 hard_violations 아님 — soft_warnings low만 허용
...
■ 절대 금지 판정:
  ...
  • 3인칭 관찰자 시점에서 "망설였다" "굳었다" "멈췄다" 등 외부 관찰 가능 행동은 시점 위반 아님
    직접 내면 확정(그녀는 X를 알고 있었다 / 그는 Y를 결심했다)만 위반 검토 대상
...
async function callLLMValidator(
  ...
  if (finishReason === "length") {
    logWarn("service:validator", "응답이 max_tokens 한도에서 잘림 — JSON 파싱 실패 가능성 높음", {
      max_tokens: MAX_TOKENS, content_len: choice?.message?.content?.length,
    });
  }
