...
[★ R5B-1.7 감정 장면화]
  ...
  - 같은 감정이 두 화 이상 유지될 때는 그 감정이 "행동에 미치는 양상"이 달라져야 한다. 예시:
    · 나쁨: "그는 여전히 불안했다." (반복)
    · 좋음: "그는 손을 거두지 않았다. 떨림은 남아 있었지만, 이번에는 먼저 룬의 빛을 가리켰다." (같은 불안 → 다른 행동)
  - 감정 라벨 단어를 본문에 직접 박지 말 것 — 독자가 행동·대사에서 추론하게 한다.
  - character_emotional_beats(있으면)의 cause/goal_delta/behavior_delta가 본문 행동·대사·결정으로 실제 구현되어야 한다.
