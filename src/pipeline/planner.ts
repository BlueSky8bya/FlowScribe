...
[★ R5B-3 발견·결말 반복 방지]
- 이미 발견된 단서·흔적·상태(이전 화에 등장 또는 summary에 기록)는 다시 "처음 발견"으로 묘사하지 않는다. 같은 단서가 다시 필요하면 _해석·추적·대응·결과·결정_ 중 하나로 전진시킨다.
- closing scene 패턴(이전 화 마지막의 결의 dialogue·인물 합심·"새로운 여정 시작"류 narrative)이 그대로 반복되지 않는다. 이번 화의 결말은 _이번 화 사건의 결과 + 다음 갈등의 씨앗_ 으로 구성한다.
- 같은 다짐 dialogue("함께 찾자"·"우리 차례야"·"새로운 세계를 향해" 등 cliché)는 한 시리즈에서 1회로 한정한다.
